...
export const checkResult = async (initialHash, counts) => {
	...
	for (let i = 100; i < payouts.length - 1; i++) {
		...
		// console.log('TREND 2')
		const bettingType2Trend = getBettingTypeTrend(TOTAL_BETTING_HISTORY2.slice(-50), 4);
		// console.log('TREND 3')
		const bettingType3Trend = getBettingTypeTrend(TOTAL_BETTING_HISTORY3.slice(-50), 4);
		// console.log('TREND 4')
		const bettingType4Trend = getBettingTypeTrend(TOTAL_BETTING_HISTORY4.slice(-50), 4);
		// console.log('TREND')
		const bettingTypeTrend = getBettingTypeTrend(TOTAL_BETTING_HISTORY.slice(-50), 4);


		let { bettingType1, bettingType2, bettingType3, bettingTypeBC, bettingType3X, trendStatus, skipCount, currentIndexLow, currentIndexHigh, currentIndexMid, bettingAvailable }
		...
		if (bettingType1 != 0) {
			...
			let isRight3X = false;
			if (nextPayout >= 3 && bettingType3X == 2) {
				isRight3X = true;
			...
			if (sortedGraph.length == 0) {
				...
				// selectedGraph = graphArray[3]

				//  const totalBettings = [TOTAL_BETTING_HISTORY1, TOTAL_BETTING_HISTORY2, TOTAL_BETTING_HISTORY3, TOTAL_BETTING_HISTORY4];

				// if (selectedGraph == null) {
				// 	selectedGraph = sortedGraph[0];
				// } else {
				// 	const prevBranch = graphArray.filter(p => p.branchIndex == selectedGraph.branchIndex)[0];
				// 	if (prevBranch.maxDeep <= 1) {
				// 		selectedGraph = prevBranch;
				// 	} else {
				// 		selectedGraph = sortedGraph[0];
				// 	}
				// }
				...
				// if (bestBranch != -1) {
				// 	pickCount++;
				// 	if (pickCount % 2 == 0 || isRightEngine) {
				// 		selectedGraph.branchIndex = bestBranch + 5;
				// 		selectedGraph.bettingType = predictResult[bestBranch] >= 2 ? 2 : 1;
				// 	} else {
				// 		selectedGraph.branchIndex = selectedGraph.branchIndex;
				// 		selectedGraph.bettingType = predictResult[selectedGraph.branchIndex - 5] >= 2 ? 2 : 1;
				// 	}
				...
				if (currentBet2X >= currentBetMaxBet2X) {
					...
					// choos bottom score
					const filteredArray = graphArray.filter(p => p.maxDeep <= 2);
					...
					if (filteredArray.length == 0) {	
						...
						selectedGraph = filteredArray[0];

						if (isRightEngine1 == false && isRightEngine2 == false && isRightEngine3 == false) {
							bettingOrNot = false;
						} else {
							selectedGraph = sortedGraph[0];
						}
					...
					// 	if (pickCount % 2 == 0 || isRightEngine) {
					// 		selectedGraph.branchIndex = bestBranch + 5;
					// 		selectedGraph.bettingType = predictResult.values[bestBranch] >= 2 ? 2 : 1;
					// 	} else {
					// 		selectedGraph.branchIndex = selectedGraph.branchIndex;
					// 		selectedGraph.bettingType = predictResult.values[selectedGraph.branchIndex - 5] >= 2 ? 2 : 1;
					// 	}
			...
			if (selectedGraph) {
				...
				bettingData.bettingType = selectedGraph.bettingType;
			...
			if (true || (i - 100) <= (currentDeadPoint + 10) && (i - 100) >= currentDeadPoint /*100000000*/ /*(deadPoints[currentDeadPoint] - 20)*/) {
			...
			if (bettingData.isRight == 1) {
				...
				if (bettingType3X == 2 && bettingTypeBC == 2) {
					if (nextPayout >= x3Payout) {
						currentAmount += currentBet3X * 3;
						totalAmount += currentBet3X * 3;
			...
			if (initialBet != 0 && currentBet2X >= initialBet * Math.pow(2, 10)) {
