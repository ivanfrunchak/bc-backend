...
export const checkResult = async (initialHash, counts) => {
	...
	for (let i = 100; i < payouts.length - 1; i++) {
		...
		if (bettingType != 0) {
			...
			let isBetting3 = !isSameFailed;// (normalCondition || anotherChangePoint);
			...
			if ((i - 100) >= 1000000000 /*(deadPoints[currentDeadPoint] - 20)*/) {
			...
			loseCount = bettingType3Trend.entireTrend2 * 3 + bettingType3Trend.engineTrend;
