	...
	for (let i = 100; i < payouts.length - 1; i++) {
		...
		if (bettingType != 0) {
			...
			if (isRight) {
				...
				if (currentBets[loseCount].real == currentBets[loseCount].startBet) {
					currentBets[loseCount].real = initialBets[loseCount];
					currentBets[loseCount].show = initialBets[loseCount];
					currentBets[loseCount].startBet = initialBets[loseCount];
				} else {


					currentBets[loseCount].startBet = currentBets[loseCount].startBet * Math.pow(2, 2);
					currentBets[loseCount].real = currentBets[loseCount].startBet;
					currentBets[loseCount].show = currentBets[loseCount].startBet;

					if (loseCount == 0 && (engineTrend == 0)) {
