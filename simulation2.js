...
export const checkResult = (initialHash, counts) => {
	...
	for (let i = 100; i < payouts.length - 1; i++) {
		...
		// }
		// if ((i - 100) % 2 == 0) {
		const currentTrainJsons = train1({ hash: hashes[i - 2] }, 150, false);
		...
		if (bettingType != 0) {
			...
			// TOTAL_BETTING_HISTORY1.push([isRight ? 1 : 0, 1, 0, [], payouts[i], bettingType, 1]);
			...
			if ((currentAmount - totalInvest - 20) < 0) {
			...
			const printCurrentBets = {};
			const printMaxBets = {};
			currentBets.map((p, i) => {
				printCurrentBets[i] = p.real;
			});

			maxLoseBets.map((p, i) => {
				printMaxBets[i] = p.show;
			});

			console.log(Math.max(...maxWinCounts), Math.max(...maxLoseCounts), JSON.stringify(loseMap), totalC)
			console.log("CURRENT LOSE COUNT: ", loseCount, "CURRENT BETS: ", printCurrentBets);
			console.log("MAX LOSE BETS", printMaxBets);
			logWarn('---------------------------------------------------------------------------------------------------------------------\n')
