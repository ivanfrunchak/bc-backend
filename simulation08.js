	...
	for (let i = 100; i < payouts.length - 1; i++) {
		...
		if (bettingType != 0) {
			...
			if (isBetting) {
				...
				if (loseCount < 4) {
					totalAmount = totalAmount - currentBets[0].real;
				} else if (loseCount == 4) {
					totalAmount = totalAmount - currentBets[4].real;
				} else {
					totalAmount = totalAmount - currentBets[0].real;
				}
			...
			if (isRight) {
				...
				if (loseCount < 4) {
					...
					totalAmount += currentBets[0].real * 2;
					currentBets[0].real = initialBets[0];
					currentBets[0].show = initialBets[0];
					
				} else if (loseCount == 4) {
					totalAmount += currentBets[4].real * 2;
				...
				if (loseCount < 4) {
					...
					if (currentBets[4].show == 1) {
						currentBets[4].show = initialBets[0] * Math.pow(2, 4);
						currentBets[4].real = initialBets[0] * Math.pow(2, 4);
					} else {
						currentBets[4].show = currentBets[4].show * 2;
						currentBets[4].real = currentBets[4].show * 2;
					}
					loseCount++;

				} else {
					currentBets[0].show = 0
					currentBets[0].real = 0
